...
void main() {
  ...
  group('NotificationService Unit Tests', () {
    test('NotificationService records sent notifications into history', () async {
      final notifService = NotificationService();
      notifService.clearHistory();

      expect(notifService.history.isEmpty, isTrue);

      await notifService.showGeofenceEnterNotification('Kadıköy Meydan', 200.0);
      ...
      await notifService.showGeofenceExitNotification('Kadıköy Meydan', 350.0);
  ...
  group('Geofence Transition & Background Tracking Tests', () {
    test('Detects entering and exiting geofence transitions and triggers notifications', () async {
      final notifService = NotificationService();
      notifService.clearHistory();
      ...
      final container = ProviderContainer(
        overrides: [
          locationStreamProvider.overrideWith((ref) => locationController.stream),
          targetLocationProvider.overrideWith((ref) => const TargetLocation(
            name: 'Kadıköy Meydan',
            latitude: 40.9905,
            longitude: 29.0255,
            radius: 200.0,
      ...
      // 1. Initial status
      expect(container.read(geofenceProvider).status, equals(GeofenceStatus.unknown));

      // 2. User location OUTSIDE geofence (~2.5 km away in Üsküdar: 41.0250, 29.0150)
      final now = DateTime.now();
      locationController.add(LocationData(
        latitude: 41.0250,
        longitude: 29.0150,
        speed: 5.0,
        accuracy: 3.0,
      ...
      expect(container.read(geofenceProvider).status, equals(GeofenceStatus.inside));
      expect(notifService.history.length, equals(notifCountBefore)); // No duplicate notification
    ...
    test('Geofence manual simulation triggers entry and exit properly', () async {
      final container = ProviderContainer();
